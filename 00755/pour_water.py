...
class Solution:
    def pourWater(self, heights: List[int], V: int, K: int) -> List[int]:
        ...
        in that direction. Also, "level" means the height of the terrain plus any water in that column.
        ...
        **Note:**
        ...
        References
        ---------
        .. [1] https://leetcode.com/problems/pour-water/discuss/171104/Share-my-Java-straightforward-and-concise-solution
        .. [2] https://www.cnblogs.com/grandyang/p/8460541.html

        """
        res = heights[:]
        for _ in range(V):
            cur = K
            while cur > 0 and res[cur] >= res[cur-1]:
                cur -= 1
            while cur < len(res)-1 and res[cur] >= res[cur+1]:
                cur += 1
            while cur > K and res[cur] == res[cur-1]:
                cur -= 1
            res[cur] += 1
        return res

    def pourWaterV01(self, heights: List[int], V: int, K: int) -> List[int]:
        res = heights[:]
        for i in range(V):
            l = r = K
            while l > 0 and res[l] >= res[l-1]:
                l -= 1
            while l < K and res[l] == res[l+1]:
                l += 1
            while r < len(res)-1 and res[r] >= res[r+1]:
                r += 1
            while r > K and res[r] == res[r-1]:
                r -= 1
            if res[l] < res[K]:
                res[l] += 1
            else:
